...
<div class="max-w-5xl mx-auto space-y-6">
    <x-breadcrumbs :items="[['label' => 'Quotations', 'url' => route('quotes.index')], ['label' => 'New Quote']]" />
    <x-sticky-form-bar :cancel-url="route('quotes.index')">
    <x-slot:title>
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Create New Quotation</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Build an itemized quotation or estimate for your customer</p>
        </div>
    </x-slot:title>
</x-sticky-form-bar>
    ...
    <form action="{{ route('quotes.store') }}" method="POST" id="quoteForm" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="quote_number" value="{{ old('quote_number', $nextNumber) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    ...
                    <input type="date" name="quote_date" value="{{ old('quote_date', date('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="expiry_date" value="{{ old('expiry_date', date('Y-m-d', strtotime('+30 days'))) }}" required
                        class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
            ...
            <div class="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                <div class="w-72 space-y-2 text-xs">
                    ...
                    <div class="flex justify-between text-slate-500 dark:text-slate-400 items-center">
                        <span>Discount:</span>
                        <div class="w-28">
                            <input type="number" name="discount" id="discountInput" value="{{ old('discount', '0.00') }}" min="0" step="0.01" oninput="calcTotals()"
        ...
        <!-- Hidden status field -->
        <input type="hidden" name="status" id="quoteStatusInput" value="sent">
