...
<head>
    ...
    <title>Print Quotation - {{ $quote->quote_number }}</title>
    <style>
        @media print {
...
<body class="bg-white text-slate-900 p-8 max-w-4xl mx-auto text-sm">
    ...
    <div class="flex justify-between items-start border-b pb-6 mb-6">
        ...
        <div class="text-right">
            <h2 class="text-2xl font-black uppercase" style="color: {{ $company->accent_color ?? '#2563eb' }};">QUOTATION</h2>
            <p class="text-sm font-mono font-bold text-slate-700 mt-1">{{ $quote->quote_number }}</p>
            <p class="text-xs text-slate-500 mt-0.5">Date: {{ date('M d, Y', strtotime($quote->quote_date)) }}</p>
            <p class="text-xs text-slate-500">Valid Until: {{ date('M d, Y', strtotime($quote->expiry_date)) }}</p>
            @if(($quote->currency_code ?? 'SGD') !== ($company->currency_code ?? 'SGD'))
                <p class="text-xs text-slate-600 font-semibold mt-1">Currency: {{ $quote->currency_code }} (Rate: {{ number_format($quote->exchange_rate, 6) }})</p>
            @endif
        </div>
    </div>

    <div class="mb-8">
        <h3 class="text-xs font-bold text-slate-400 uppercase mb-1">Quoted To</h3>
        <h4 class="text-sm font-bold text-slate-900">{{ $quote->customer->name ?? 'Customer' }}</h4>
        <p class="text-xs text-slate-600">{{ $quote->customer->address ?? '' }}</p>
        <p class="text-xs text-slate-600">{{ $quote->customer->city ?? '' }}, {{ $quote->customer->country ?? '' }}</p>
        <p class="text-xs text-slate-600">Email: {{ $quote->customer->email ?? '' }}</p>
    </div>
    ...
    <div class="flex justify-end mb-8">
        <div class="w-64 space-y-1.5 text-xs">
            ...
            <div class="flex justify-between text-slate-600">
                <span>Tax (GST):</span>
                <span>{{ $currencySymbol }}{{ number_format($quote->tax_total, 2) }}</span>
            </div>
            @if(($quote->discount ?? 0) > 0)
            <div class="flex justify-between text-slate-600">
                <span>Discount:</span>
                <span class="text-red-600">-{{ $currencySymbol }}{{ number_format($quote->discount, 2) }}</span>
            </div>
            @endif
            <div class="flex justify-between text-base font-bold text-slate-900 pt-2 border-t">
                <span>Quote Total:</span>
                <span>{{ $currencySymbol }}{{ number_format($quote->total, 2) }}</span>
            </div>
            @if(($quote->currency_code ?? 'SGD') !== ($company->currency_code ?? 'SGD'))
            <div class="flex justify-between text-xs text-slate-600 pt-1 border-t mt-1">
                <span>{{ $company->currency_code ?? 'SGD' }} Equivalent:</span>
                <span class="font-semibold">{{ $company->currency_symbol ?? 'S$' }}{{ number_format($quote->total * $quote->exchange_rate, 2) }}</span>
            </div>
            @endif
        </div>
    </div>
    ...
    @if($quote->terms)
        <div class="border-t pt-4 mt-4 text-xs text-slate-600">
            <h4 class="font-bold text-slate-800 mb-1">Terms & Conditions:</h4>
