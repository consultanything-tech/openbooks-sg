...
<div class="max-w-5xl mx-auto space-y-6">
    <x-sticky-form-bar :cancel-url="route('credit_notes.index')">
    <x-slot:title>
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Create Credit Note</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Issue a credit note for returns, overcharges, or corrections</p>
        </div>
    </x-slot:title>
</x-sticky-form-bar>
    ...
    <form action="{{ route('credit_notes.store') }}" method="POST" id="creditNoteForm" class="space-y-6">
        @csrf

        <!-- Top Details Card -->
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            <h2 class="text-sm font-bold text-slate-900 dark:text-white mb-1">Credit Note Details</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-5">Customer, reference invoice, and date</p>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Customer *</label>
                    <select data-combobox name="customer_id" id="customerSelect" required onchange="filterInvoices()" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        <option value="">Select Customer</option>
                        @foreach($customers as $c)
                            <option value="{{ $c->id }}" {{ old('customer_id', $invoice->customer_id ?? '') == $c->id ? 'selected' : '' }}>{{ $c->name }} ({{ $c->email ?: 'No email' }})</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Credit Note Number *</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Reference Invoice</label>
                    <select data-combobox name="invoice_id" id="invoiceSelect" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        <option value="">None (standalone credit note)</option>
                        @foreach($invoices as $inv)
                            <option value="{{ $inv->id }}" data-customer="{{ $inv->customer_id }}" {{ old('invoice_id', $invoice->id ?? '') == $inv->id ? 'selected' : '' }}>
                                {{ $inv->invoice_number }} - {{ $inv->customer->name ?? '' }} ({{ $currencySymbol }}{{ number_format($inv->total, 2) }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Credit Note Date *</label>
                    <input type="date" name="credit_note_date" value="{{ old('credit_note_date', date('Y-m-d')) }}" required
                ...
                <div class="md:col-span-2">
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Reason for Credit Note</label>
                    <input type="text" name="reason" value="{{ old('reason') }}" placeholder="e.g. Returned goods, billing correction, service not rendered..."
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Items to credit back to the customer</p>
                </div>
                ...
                <table class="w-full text-left text-xs" id="itemsTable">
                    ...
                    <tbody id="itemsBody" class="divide-y divide-slate-100 dark:divide-slate-800">
                        @if($invoiceItems && count($invoiceItems) > 0)
                            @foreach($invoiceItems as $idx => $lineItem)
                            <tr class="item-row">
                                <td class="py-2.5 px-3">
                                    <div class="space-y-1.5">
                                        <select onchange="onCatalogItemSelect(this)" class="catalog-select w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-slate-700 dark:text-slate-300 text-xs font-medium focus:outline-none focus:border-indigo-500 appearance-none cursor-pointer">
                                            ...
                                            @foreach($items as $it)
                                                <option value="{{ $it->id }}" data-name="{{ $it->name }}" data-price="{{ $it->sale_price }}" data-tax-rate="{{ $it->tax->rate ?? 0 }}" {{ $lineItem->item_id == $it->id ? 'selected' : '' }}>
                                                    {{ $it->name }} {{ $it->sku ? '(' . $it->sku . ')' : '' }} &bull; {{ $currencySymbol }}{{ number_format($it->sale_price, 2) }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <input type="text" name="items[{{ $idx }}][item_name]" value="{{ $lineItem->name }}" list="catalogItemsList" required placeholder="Service or Product description"
                                ...
                                <td class="py-2.5 px-3 align-top pt-3">
                                    <input type="number" name="items[{{ $idx }}][quantity]" value="{{ $lineItem->quantity }}" min="1" step="1" required oninput="calcTotals()"
                                ...
                                <td class="py-2.5 px-3 align-top pt-3">
                                    <input type="number" name="items[{{ $idx }}][price]" value="{{ number_format($lineItem->price, 2, '.', '') }}" min="0" step="0.01" required oninput="calcTotals()"
                            ...
                            <td class="py-2.5 px-3">
                                <div class="space-y-1.5">
                                    <select onchange="onCatalogItemSelect(this)" class="catalog-select w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-slate-700 dark:text-slate-300 text-xs font-medium focus:outline-none focus:border-indigo-500 appearance-none cursor-pointer">
                                        <option value="">-- Choose from Catalog (or type below) --</option>
                                        @foreach($items as $it)
                                            <option value="{{ $it->id }}" data-name="{{ $it->name }}" data-price="{{ $it->sale_price }}" data-tax-rate="{{ $it->tax->rate ?? 0 }}">
                                                {{ $it->name }} {{ $it->sku ? '(' . $it->sku . ')' : '' }} &bull; {{ $currencySymbol }}{{ number_format($it->sale_price, 2) }}
                                            </option>
                                        @endforeach
                                    </select>
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            <div>
                ...
                <textarea name="notes" rows="2" placeholder="Additional notes for the credit note..."
        ...
        <!-- Hidden status field -->
        <input type="hidden" name="status" id="creditNoteStatusInput" value="issued">

        <!-- Submission Buttons -->
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('credit_notes.index') }}" class="btn btn-ghost">Cancel</a>
            <button type="submit" onclick="document.getElementById('creditNoteStatusInput').value='draft'" class="btn btn-secondary">
                <i data-lucide="save" aria-hidden="true"></i> Save as Draft
            </button>
            <button type="submit" onclick="document.getElementById('creditNoteStatusInput').value='issued'" id="primarySubmit" class="btn btn-primary">
...
@section('scripts')
<script>
    let rowIndex = {{ ($invoiceItems && count($invoiceItems) > 0) ? count($invoiceItems) : 1 }};
    ...
    function filterInvoices() {
        ...
        allInvoices.forEach(inv => {
            if (!customerId || inv.customer_id == customerId) {
                ...
                opt.value = inv.id;
                opt.textContent = inv.invoice_number + ' - ' + (inv.customer ? inv.customer.name : '') + ' (' + currencySymbol + parseFloat(inv.total).toFixed(2) + ')';
                opt.dataset.customer = inv.customer_id;
                if (inv.id == currentVal) opt.selected = true;
    ...
    function onCatalogItemSelect(selectElem) {
        ...
        if (taxSelect) {
            ...
            for (let opt of taxSelect.options) {
                if (parseFloat(opt.value) === taxRate) { opt.selected = true; matched = true; break; }
            }
            if (!matched && taxSelect.options.length > 0) taxSelect.options[0].selected = true;
        }
        calcTotals();
    }
    ...
    function onItemNameInput(inputElem) {
        ...
        if (matchedItem) {
            ...
            if (taxSelect && matchedItem.tax) {
                for (let opt of taxSelect.options) {
                    if (parseFloat(opt.value) === parseFloat(matchedItem.tax.rate)) { opt.selected = true; break; }
                }
            }
            calcTotals();
        }
    ...
    function addRow() {
        ...
        taxes.forEach(t => { taxOptions += `<option value="${t.rate}">${t.name} (${t.rate}%)</option>`; });
    ...
    function removeRow(btn) {
        ...
        else { alert('Credit note must contain at least one line item.'); }
    }
