...
<div class="max-w-5xl mx-auto space-y-6">
    <!-- Header -->
    <x-sticky-form-bar :cancel-url="route('credit_notes.show', $creditNote->id)">
    <x-slot:title>
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Credit Note #{{ $creditNote->credit_note_number }}</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Update credit note details, line items, and totals</p>
        </div>
    </x-slot:title>
</x-sticky-form-bar>
    ...
    <form action="{{ route('credit_notes.update', $creditNote->id) }}" method="POST" id="creditNoteForm" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
                ...
                <div>
                    ...
                    <select data-combobox name="customer_id" id="customerSelect" required onchange="filterInvoices()" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        ...
                        @foreach($customers as $c)
                            <option value="{{ $c->id }}" {{ old('customer_id', $creditNote->customer_id) == $c->id ? 'selected' : '' }}>{{ $c->name }} ({{ $c->email ?: 'No email' }})</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Credit Note Number *</label>
                    <input type="text" name="credit_note_number" value="{{ old('credit_note_number', $creditNote->credit_note_number) }}" required
                ...
                <div>
                    ...
                    <select data-combobox name="invoice_id" id="invoiceSelect" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        ...
                        @foreach($invoices as $inv)
                            <option value="{{ $inv->id }}" data-customer="{{ $inv->customer_id }}" {{ old('invoice_id', $creditNote->invoice_id) == $inv->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    ...
                    <input type="date" name="credit_note_date" value="{{ old('credit_note_date', $creditNote->credit_note_date instanceof \Carbon\Carbon ? $creditNote->credit_note_date->format('Y-m-d') : $creditNote->credit_note_date) }}" required
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="reason" value="{{ old('reason', $creditNote->reason) }}" placeholder="e.g. Returned goods, billing correction..."
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs" id="itemsTable">
                    ...
                    <tbody id="itemsBody" class="divide-y divide-slate-100 dark:divide-slate-800">
                        @foreach($creditNote->items as $idx => $lineItem)
                        <tr class="item-row">
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            <div>
                ...
                <textarea name="notes" rows="2" placeholder="Additional notes for the credit note..."
                    class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">{{ old('notes', $creditNote->notes) }}</textarea>
            </div>
        </div>

        <!-- Hidden status field -->
        <input type="hidden" name="status" id="creditNoteStatusInput" value="draft">

        <!-- Submission Buttons -->
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('credit_notes.show', $creditNote->id) }}" class="btn btn-ghost">Cancel</a>
...
@section('scripts')
<script>
    let rowIndex = {{ count($creditNote->items) }};
