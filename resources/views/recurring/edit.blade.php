...
<div class="max-w-5xl mx-auto space-y-6">
    <x-sticky-form-bar :cancel-url="route('recurring.show', $template->id)">
    <x-slot:title>
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Recurring Template</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Update the schedule, items, or recipient for this recurring template</p>
        </div>
    </x-slot:title>
</x-sticky-form-bar>
    ...
    <form action="{{ route('recurring.update', $template->id) }}" method="POST" id="recurringForm" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="type" id="templateType" onchange="toggleRecipient()" required class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        <option value="invoice" {{ $template->type === 'invoice' ? 'selected' : '' }}>Invoice (Customer)</option>
                        <option value="bill" {{ $template->type === 'bill' ? 'selected' : '' }}>Bill (Vendor)</option>
                    </select>
                </div>
                <div id="customerField" style="{{ $template->type === 'bill' ? 'display:none' : '' }}">
                    ...
                    <select data-combobox name="customer_id" id="customerSelect" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        ...
                        @foreach($customers as $c)
                            <option value="{{ $c->id }}" {{ $template->customer_id == $c->id ? 'selected' : '' }}>{{ $c->name }} ({{ $c->email ?: 'No email' }})</option>
                        @endforeach
                    </select>
                </div>
                <div id="vendorField" style="{{ $template->type === 'invoice' ? 'display:none' : '' }}">
                    ...
                    <select data-combobox name="vendor_id" id="vendorSelect" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        ...
                        @foreach($vendors as $v)
                            <option value="{{ $v->id }}" {{ $template->vendor_id == $v->id ? 'selected' : '' }}>{{ $v->name }} ({{ $v->email ?: 'No email' }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Next Due Date *</label>
                    <input type="date" name="next_due_date" value="{{ $template->next_due_date->format('Y-m-d') }}" required
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Items included on each generated invoice/bill</p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs" id="itemsTable">
                    ...
                    <tbody id="itemsBody" class="divide-y divide-slate-100 dark:divide-slate-800">
                        @foreach($template->items as $idx => $lineItem)
                        <tr class="item-row">
                            ...
                            <td class="py-2.5 px-3 align-top pt-3">
                                <input type="number" name="items[{{ $idx }}][price]" value="{{ $lineItem->price }}" min="0" step="0.01" required oninput="calcTotals()"
                            ...
                            <td class="py-2.5 px-3 align-top pt-4 text-right font-extrabold text-slate-900 dark:text-white line-total">{{ $currencySymbol }}{{ number_format($lineItem->total, 2) }}</td>
            ...
            <div class="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                <div class="w-72 space-y-2 text-xs">
                    <div class="flex justify-between text-slate-500 dark:text-slate-400">
                        <span>Subtotal:</span>
                        <span id="subtotalDisplay" class="text-slate-900 dark:text-white font-bold">{{ $currencySymbol }}{{ number_format($template->subtotal, 2) }}</span>
                    </div>
                    <div class="flex justify-between text-slate-500 dark:text-slate-400">
                        <span>Taxes (GST/VAT):</span>
                        <span id="taxDisplay" class="text-slate-900 dark:text-white font-bold">{{ $currencySymbol }}{{ number_format($template->tax_total, 2) }}</span>
                    </div>
                    <div class="flex justify-between text-slate-500 dark:text-slate-400 items-center">
                        <span>Discount:</span>
                        <div class="w-28">
                            <input type="number" name="discount" id="discountInput" value="{{ number_format($template->discount_total, 2, '.', '') }}" min="0" step="0.01" oninput="calcTotals()"
                    ...
                    <div class="flex justify-between text-base font-bold text-slate-900 dark:text-white pt-2 border-t border-slate-200 dark:border-slate-700">
                        <span>Total:</span>
                        <span id="grandTotalDisplay" class="text-indigo-700 dark:text-indigo-400 font-black">{{ $currencySymbol }}{{ number_format($template->total, 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="terms" rows="2" placeholder="Payment terms..."
                    class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">{{ $template->terms }}</textarea>
            </div>
        </div>

        <!-- Buttons -->
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('recurring.show', $template->id) }}" class="btn btn-ghost">Cancel</a>
...
@section('scripts')
<script>
    let rowIndex = {{ count($template->items) }};
