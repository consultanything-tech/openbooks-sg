...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <x-slot:title>
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Time Entry</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Update the details for this time tracking entry</p>
        </div>
    </x-slot:title>
</x-sticky-form-bar>
    ...
    @canEdit
    <form action="{{ route('time_tracking.update', $entry->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-5">Modify the time tracking information below</p>
                ...
                <div>
                    ...
                    <select data-combobox name="customer_id" required class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        ...
                        @foreach($customers as $customer)
                            <option value="{{ $customer->id }}" {{ old('customer_id', $entry->customer_id) == $customer->id ? 'selected' : '' }}>{{ $customer->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Project</label>
                    <input type="text" name="project" value="{{ old('project', $entry->project) }}" placeholder="e.g. Website Redesign"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    ...
                    <input type="date" name="entry_date" value="{{ old('entry_date', $entry->entry_date) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="hours" id="hoursInput" value="{{ old('hours', $entry->hours) }}" step="0.25" min="0" required placeholder="0.00"
                ...
                <div>
                    ...
                    <input type="number" name="rate" id="rateInput" value="{{ old('rate', $entry->rate) }}" step="0.01" min="0" required placeholder="0.00"
                        oninput="updateAmountPreview()"
                        class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white font-bold placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
                ...
                <div>
                    ...
                    <p id="amountPreview" class="text-lg font-extrabold text-slate-900 dark:text-white">{{ $currencySymbol ?? '' }}{{ number_format($entry->amount, 2) }}</p>
                </div>
            </div>
            ...
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800">
                <label class="flex items-center gap-2.5 cursor-pointer">
                    <input type="checkbox" name="is_billable" value="1" {{ old('is_billable', $entry->is_billable) ? 'checked' : '' }}
