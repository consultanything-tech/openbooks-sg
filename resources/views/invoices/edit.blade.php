...
<div class="max-w-5xl mx-auto space-y-6">
    <x-breadcrumbs :items="[['label' => 'Invoices', 'url' => route('invoices.index')], ['label' => $invoice->invoice_number, 'url' => route('invoices.show', $invoice->id)], ['label' => 'Edit']]" />
    <!-- Header -->
    <x-sticky-form-bar :cancel-url="route('invoices.show', $invoice->id)">
    <x-slot:title>
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Invoice #{{ $invoice->invoice_number }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Update invoice details, line items, and totals</p>
        </div>
    </x-slot:title>
</x-sticky-form-bar>
    ...
    <form action="{{ route('invoices.update', $invoice->id) }}" method="POST" id="invoiceForm" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Invoice Number *</label>
                    ...
                    @error('invoice_number')<p class="mt-1 text-xs text-red-600 dark:text-red-400" role="alert">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Order / PO Reference</label>
                    <input type="text" name="order_number" value="{{ old('order_number', $invoice->order_number) }}" placeholder="e.g. PO-9821"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Invoice Date *</label>
                    <input type="date" name="invoice_date" value="{{ old('invoice_date', $invoice->invoice_date instanceof \Carbon\Carbon ? $invoice->invoice_date->format('Y-m-d') : $invoice->invoice_date) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="due_date" value="{{ old('due_date', $invoice->due_date instanceof \Carbon\Carbon ? $invoice->due_date->format('Y-m-d') : $invoice->due_date) }}" required
                        class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    ...
                    <select name="currency_code" id="currencyCodeSelect" onchange="onCurrencyChange()" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        <option value="SGD" data-rate="1.000000" data-symbol="{{ $company->currency_symbol ?? 'S$' }}" {{ ($invoice->currency_code ?? 'SGD') === 'SGD' ? 'selected' : '' }}>{{ $company->currency_code ?? 'SGD' }} ({{ $company->currency_symbol ?? 'S$' }}) - Base Currency</option>
                        @foreach($currencies as $cur)
                            <option value="{{ $cur->currency_code }}" data-rate="{{ $cur->exchange_rate }}" data-symbol="{{ $cur->currency_symbol }}" {{ ($invoice->currency_code ?? 'SGD') == $cur->currency_code ? 'selected' : '' }}>
                ...
                <div id="exchangeRateField" class="hidden">
                    ...
                    <input type="number" name="exchange_rate" id="exchangeRateInput" value="{{ old('exchange_rate', $invoice->exchange_rate ?? '1.000000') }}" step="0.000001" min="0.000001"
                        oninput="calcTotals()"
                        class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white font-mono font-bold placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
            ...
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800">
                <div class="inline-flex items-center gap-2 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-xl px-4 py-2">
                    <i data-lucide="check-circle" class="w-4 h-4 text-emerald-600 dark:text-emerald-400"></i>
                    <span class="text-xs font-semibold text-emerald-700 dark:text-emerald-400">Paid Amount: {{ $currencySymbol }}{{ number_format($invoice->paid_amount, 2) }}</span>
                </div>
            </div>
            @endif
        </div>
            ...
            <div class="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                <div class="w-72 space-y-2 text-xs">
                    ...
                    <div class="flex justify-between text-slate-500 dark:text-slate-400 items-center">
                        <span>Discount:</span>
                        <div class="w-28">
                            <input type="number" name="discount" id="discountInput" value="{{ old('discount', number_format($invoice->discount ?? 0, 2, '.', '')) }}" min="0" step="0.01" oninput="calcTotals()"
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            <div>
                <label class="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Customer Notes / Payment Instructions</label>
                <textarea name="notes" rows="2" placeholder="Bank details, PayNow/GIRO instructions, or thank you message..."
                    class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">{{ old('notes', $invoice->notes) }}</textarea>
            ...
            <div>
                ...
                <textarea name="terms" rows="2" placeholder="Payment terms, late fees, warranty info..."
                    class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">{{ old('terms', $invoice->terms) }}</textarea>
                @error('terms')<p class="mt-1 text-xs text-red-600 dark:text-red-400" role="alert">{{ $message }}</p>@enderror
            </div>
        </div>

        <!-- Hidden status field -->
        <input type="hidden" name="status" id="invoiceStatusInput" value="{{ $invoice->status }}">

        <!-- Submission Buttons -->
        <div class="flex items-center justify-end gap-4">
            <a href="{{ route('invoices.show', $invoice->id) }}" class="btn btn-ghost">Cancel</a>
            @if($invoice->status === 'draft')
...
<script>
    ...
    function onCurrencyChange() {
        ...
        if (code !== baseCurrencyCode) {
            ...
            // Only update rate from dropdown if user just changed currency (not on initial load with saved rate)
            if (!window._currencyInitialized) {
                rateInput.value = {{ $invoice->exchange_rate ?? 1 }};
