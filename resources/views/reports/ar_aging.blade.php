...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Accounts Receivable Aging</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Outstanding customer invoices grouped by aging buckets &bull; As of {{ $today->format('M d, Y') }}</p>
            <a href="{{ route('reports.ar_aging.export_csv') }}" class="btn btn-secondary mt-2">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <a href="{{ route('reports.ar_aging') }}" class="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20 shadow-sm">AR Aging</a>
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <h2 class="text-sm font-bold text-slate-900 dark:text-white">Customer Aging Breakdown</h2>
            <span class="text-[11px] text-slate-400 dark:text-slate-500 font-medium">{{ count($customerBreakdown) }} customer(s) with outstanding balances</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60">
                        <th class="text-left px-6 py-3 font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider text-[10px]">Customer</th>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-slate-400 dark:text-slate-500">
                                ...
                                <p class="font-medium">No outstanding receivables</p>
                                <p class="text-[11px] mt-0.5">All customer invoices are fully paid.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                @if(count($customerBreakdown) > 0)
                <tfoot>
