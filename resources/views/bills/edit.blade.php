...
<div class="max-w-5xl mx-auto space-y-6">
    <x-breadcrumbs :items="[['label' => 'Bills', 'url' => route('bills.index')], ['label' => $bill->bill_number, 'url' => route('bills.show', $bill->id)], ['label' => 'Edit']]" />
    <x-sticky-form-bar :cancel-url="route('bills.index')">
    <x-slot:title>
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Vendor Bill</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Update bill <span class="font-mono font-bold">{{ $bill->bill_number }}</span></p>
        </div>
    </x-slot:title>
</x-sticky-form-bar>
    ...
    <form action="{{ route('bills.update', $bill->id) }}" method="POST" id="billForm" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    ...
                    <input type="date" name="bill_date" value="{{ old('bill_date', $bill->bill_date instanceof \Carbon\Carbon ? $bill->bill_date->format('Y-m-d') : $bill->bill_date) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="due_date" value="{{ old('due_date', $bill->due_date instanceof \Carbon\Carbon ? $bill->due_date->format('Y-m-d') : $bill->due_date) }}" required
                        class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                    ...
                    <select name="currency_code" id="currencyCodeSelect" onchange="onCurrencyChange()" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white appearance-none cursor-pointer focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                        <option value="SGD" data-rate="1.000000" data-symbol="{{ $company->currency_symbol ?? 'S$' }}" {{ ($bill->currency_code ?? 'SGD') === 'SGD' ? 'selected' : '' }}>{{ $company->currency_code ?? 'SGD' }} ({{ $company->currency_symbol ?? 'S$' }}) - Base Currency</option>
                        @foreach($currencies as $cur)
                            <option value="{{ $cur->currency_code }}" data-rate="{{ $cur->exchange_rate }}" data-symbol="{{ $cur->currency_symbol }}" {{ ($bill->currency_code ?? 'SGD') == $cur->currency_code ? 'selected' : '' }}>
                ...
                <div id="exchangeRateField" class="hidden">
                    ...
                    <input type="number" name="exchange_rate" id="exchangeRateInput" value="{{ old('exchange_rate', $bill->exchange_rate ?? '1.000000') }}" step="0.000001" min="0.000001"
                        oninput="calcTotals()"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs" id="itemsTable">
                    ...
                    <tbody id="itemsBody" class="divide-y divide-slate-100 dark:divide-slate-800">
                        @foreach($bill->items as $idx => $lineItem)
                        <tr class="item-row">
                            <td class="py-2.5 px-3">
                                <div class="space-y-1.5">
                                    ...
                                    <input type="text" name="items[{{ $idx }}][item_name]" value="{{ old('items.' . $idx . '.item_name', $lineItem->name) }}" list="billCatalogItemsList" required placeholder="Service or Purchased item"
        ...
        <div class="flex items-center justify-between gap-4">
            <button type="button" onclick="obConfirm('Are you sure you want to delete this bill? This action cannot be undone.', { title: 'Delete bill', confirmLabel: 'Delete' }).then(function(ok){ if (ok) document.getElementById('deleteBillForm').submit(); })" class="btn btn-danger-text">
                Delete Bill
            </button>
            <!-- Hidden status field -->
            <input type="hidden" name="status" id="billStatusInput" value="{{ $bill->status }}">
    ...
    <!-- Delete Form -->
    <form id="deleteBillForm" action="{{ route('bills.destroy', $bill->id) }}" method="POST" class="hidden" onsubmit="return confirm('Are you sure you want to delete this bill? This action cannot be undone.')">
        @csrf
        @method('DELETE')
    </form>
</div>
    ...
    function onCurrencyChange() {
        ...
        if (code !== baseCurrencyCode) {
            ...
            if (!window._currencyInitialized) {
                rateInput.value = {{ $bill->exchange_rate ?? 1 }};
